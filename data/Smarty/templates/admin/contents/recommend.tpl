...
<div id="admin-contents" class="contents-main">
    ...
    <div class="recommend-product <!--{if $arrItems[$smarty.section.cnt.iteration].status == "2"}-->hidden<!--{/if}-->">
        <table class="list center recommend-table">
            ...
            <tr>
                <td>おすすめ求人(<!--{$smarty.section.cnt.iteration}-->)</td>
                            ...
                            <div class="clearfix table-wrap">
                                ...
                                <div class="table-detail">
                                    <div class="detail-name">
                                        求人名： <!--{$arrItems[$smarty.section.cnt.iteration].name|h}-->
                                    </div>
                                    <div class="detail-form">
                                        <form name="form<!--{$smarty.section.cnt.iteration}-->" id="form<!--{$smarty.section.cnt.iteration}-->" method="post" action="?#recommend_<!--{$smarty.section.cnt.iteration}-->">
                                            ...
                                            <br />
                                            <span class="attention"><!--{$arrErr[$smarty.section.cnt.iteration].comment_vn}--></span>
                                            <textarea class="vn_field" name="comment_vn" cols="45" rows="4" style="width: 586px; height: 80px; <!--{$arrErr[$smarty.section.cnt.iteration].comment_vn|sfGetErrorColor}-->" <!--{$arrItems[$smarty.section.cnt.iteration].product_id|sfGetEnabled}-->><!--{"\n"}--><!--{$arrItems[$smarty.section.cnt.iteration].comment_vn|h}--></textarea>
                                        </form>
                                    </div>
                                </div>
                            </div>
                    ...
                    <!--{else}-->
                        <td class="AlignLeft">
                            <a class="btn-action-m" href="javascript:;" onclick="lfnCheckSetItem('<!--{$smarty.section.cnt.iteration}-->'); return false;" target="_blank"><span class="btn-next">求人を選択する</span></a>
                            <form name="form<!--{$smarty.section.cnt.iteration}-->" id="form<!--{$smarty.section.cnt.iteration}-->" method="post" action="?#recommend_<!--{$smarty.section.cnt.iteration}-->">
                                <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                                <input type="hidden" name="mode" value="regist" />
                                <input type="hidden" name="best_id" value="<!--{$arrItems[$smarty.section.cnt.iteration].best_id|h}-->" />
    ...
    <!--▲おすすめ求人<!--{$smarty.section.cnt.iteration}-->-->
    <!--{/section}-->
</div>
