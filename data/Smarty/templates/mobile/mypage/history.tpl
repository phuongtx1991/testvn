...
<!--{strip}-->
    ...
    <!--{foreach from=$tpl_arrOrderDetail item=orderDetail}-->
        <hr>
        仕事コード：<!--{$orderDetail.product_code|h}--><br>
        仕事名：<a<!--{if $orderDetail.enable}--> href="<!--{$smarty.const.P_DETAIL_URLPATH}--><!--{$orderDetail.product_id|u}-->"<!--{/if}-->><!--{$orderDetail.product_name|h}--></a><br>
        仕事種別：
    ...
    <!--{foreach item=shippingItem name=shippingItem from=$arrShipping}-->
        ...
        <!--{if $isMultiple}-->
            <!--{foreach item=item from=$shippingItem.shipment_item}-->
                仕事コード：<!--{$item.productsClass.product_code|h}--><br>
                仕事名：<!--{* 仕事名 *}--><!--{$item.productsClass.name|h}--><br>
