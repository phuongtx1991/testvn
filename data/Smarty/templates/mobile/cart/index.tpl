...
<!--{strip}-->
        ...
        <!--{foreach from=$cartKeys item=key}-->
            <form name="form<!--{$key}-->" id="form<!--{$key}-->" method="post" action="?" utn>
                ...
                <!--{foreach from=$cartItems[$key] item=item}-->
                    ◎<!--{* 仕事名 *}--><!--{$item.productsClass.name|h}--><br>
                    ...
                    <div align="right"><a href="<!--{$smarty.const.MOBILE_P_DETAIL_URLPATH}--><!--{$item.productsClass.product_id|u}-->">→仕事詳細へ</a></div>
                    <HR>
                <!--{/foreach}-->
                <font color="#FF0000">
    ...
    <!--{if $smarty.const.USE_POINT !== false}-->
        ...
        <!--{/if}-->
        ポイントは仕事購入時に1ptを<!--{$smarty.const.POINT_VALUE}-->円として使用することができます。
    <!--{/if}-->
<!--{/strip}-->
