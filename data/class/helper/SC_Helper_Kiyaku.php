...
class SC_Helper_Kiyaku
{
    ...
    {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $col = 'kiyaku_id, kiyaku_title, kiyaku_title_vn, kiyaku_text_vn';
        $where = '';
        if (!$has_deleted) {
            $where .= 'del_flg = 0';
        }
    ...
    public function isTitleExist($title, $kiyaku_id = NULL)
    {
        ...
        $where  = 'del_flg = 0 AND kiyaku_title_vn = ?';
        $arrVal = array($title);
        ...
        $arrRet = $objQuery->select('kiyaku_id, kiyaku_title_vn', 'dtb_kiyaku', $where, $arrVal);
