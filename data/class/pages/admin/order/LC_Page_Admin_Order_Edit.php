...
class LC_Page_Admin_Order_Edit extends LC_Page_Admin_Order_Ex
{
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        // 受注詳細情報
        $objFormParam->addParam('求人種別ID', 'product_type_id', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), '0');
        ...
        $objFormParam->addParam('求人ID', 'product_id', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), '0');
        $objFormParam->addParam('求人規格ID', 'product_class_id', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), '0');
        $objFormParam->addParam('ポイント付与率', 'point_rate');
        $objFormParam->addParam('求人コード', 'product_code');
        $objFormParam->addParam('求人名', 'product_name');
        ...
        $objFormParam->addParam('求人規格ID', 'shipment_product_class_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('求人コード', 'shipment_product_code');
        $objFormParam->addParam('求人名', 'shipment_product_name');
        ...
        $objFormParam->addParam('求人項番', 'no', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('追加求人規格ID', 'add_product_class_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('修正求人規格ID', 'edit_product_class_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
    ...
    {
        $objFormParam->addParam('求人規格ID', 'multiple_product_class_id', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('求人コード', 'multiple_product_code', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), 1);
        $objFormParam->addParam('求人名', 'multiple_product_name', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), 1);
    ...
    public function setMultipleItemTo(&$objFormParam)
    {
         ...
         *
         * $arrShipmentItem[お届け先ID][求人規格ID]['shipment_(key)'] = 値
         */
         ...
         * $arrProductQuantity[$shipping_id] = お届け先ごとの配送求人数量
         */
    ...
    public function setOrderToFormParam(&$objFormParam, $order_id)
    {
        ...
        if ($arrShippingsTmp) {
            ...
            // ダウンロード求人の場合はお届け先情報がないので受注詳細から必要なデータを挿入する
    ...
    public function doRegister($order_id, &$objPurchase, &$objFormParam, &$message, &$arrValuesBefore)
    {
        ...
        // 変更しようとしている求人情報とDBに登録してある求人情報を比較することで、更新すべき数量を計算
        $max = count($arrDetail);
        $k = 0;
        $arrStockData = array();
        for ($i = 0; $i < $max; $i++) {
        ...
        // 上記の新しい求人のループでDELETEされなかった求人は、注文より削除された求人
     ...
     * 受注求人を削除する.
     *
     * @param  integer      $delete_no    削除する受注求人の項番
     * @param  SC_FormParam $objFormParam SC_FormParam インスタンス
     * @return void
     */
     ...
     *
     * @param  SC_FormParam $objFormParam         SC_FormParam インスタンス
     * @param  integer      $add_product_class_id 追加求人規格ID
     * @return void
     */
     ...
     *
     * @param  SC_FormParam $objFormParam         SC_FormParam インスタンス
     * @param  integer      $edit_product_class_id 変更求人規格ID
    ...
    public function shipmentEditProduct(&$objFormParam, $edit_product_class_id, $change_no)
    {
        ...
        if ($pre_shipment_product_class_id == $edit_product_class_id) {
            ...
            // 求人規格の変更によって求人の重複が発生する場合は一つにまとめる
     ...
     *
     * @param  SC_FormParam $objFormParam                               SC_FormParam インスタンス
     * @param  array        $arrProductClassIds　                      削除対象配列の求人規格ID
     * @param  integer      $delete_product_class_id　削除求人規? ?ID
     ...
     *
     * @param SC_FormParam  $objFormParam               SC_FormParam インスタンス
     * @param array         $arrProductClassIds 　      対象配列の求人規格ID
     * @param int           $insert_product_class_id    追加求人規格ID
    ...
    public function checkInsertOrderProducts(&$objFormParam, $arrProductClassIds, $insert_product_class_id, $arrAddProductInfo)
    {
        if (!$arrProductClassIds || !in_array($insert_product_class_id, $arrProductClassIds)) {
            ...
            //受注求人の数量は、複数配送側の集計で出しているので、重複しても数量を増やさない。
            return null;
        }
    }
}
