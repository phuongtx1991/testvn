...
class LC_Page_Admin_Products_ProductClass extends LC_Page_Admin_Ex
{
    ...
    public function initParam(&$objFormParam)
    {
        ...
        // 求人規格
        $objFormParam->addParam('求人規格数', 'total', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('求人名', 'product_name', STEXT_LEN, 'KVa', array('MAX_LENGTH_CHECK'));
        $objFormParam->addParam('求人コード', 'product_code', STEXT_LEN, 'KVa', array('MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('求人規格ID', 'product_class_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        ...
        $objFormParam->addParam('求人種別', 'product_type_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
     ...
     * @param integer $product_id 登録を行う求人ID
     */
    public function registerProductClass($arrList, $product_id, $total)
    {
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            /*
             * チェックを入れない求人は product_type_id が NULL になるので, 0 を入れる
             */
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if (!SC_Utils_Ex::isBlank($arrValues['check'][$i])) {
                ...
                if (SC_Utils_Ex::isBlank($arrValues['product_type_id'][$i])) {
                    $arrErr['product_type_id'][$i] = '※ 求人種別は、いずれかを選択してください。<br />';
                }
                /*
                 * ダウンロード求人の必須チェック
                 */
                if ($arrValues['product_type_id'][$i] == PRODUCT_TYPE_DOWNLOAD) {
                    if (SC_Utils_Ex::isBlank($arrValues['down_filename'][$i])) {
                        $arrErr['down_filename'][$i] = '※ ダウンロード求人の場合はダウンロードファイル名を入力してください。<br />';
                    }
                    if (SC_Utils_Ex::isBlank($arrValues['down_realfilename'][$i])) {
                        $arrErr['down_realfilename'][$i] = '※ ダウンロード求人の場合はダウンロード求人用ファイルをアップロードしてください。<br />';
                    }
                /*
                 * 通常求人チェック
                 */
                    ...
                    if (!SC_Utils_Ex::isBlank($arrValues['down_filename'][$i])) {
                        $arrErr['down_filename'][$i] = '※ ダウンロード求人ではない場合、ダウンロードファイル名を設定できません。<br />';
                    }
                    if (!SC_Utils_Ex::isBlank($arrValues['down_realfilename'][$i])) {
                        $arrErr['down_realfilename'][$i] = '※ ダウンロード求人ではない場合、ダウンロード求人用ファイルをアップロードできません。<br />ファイルを取り消してください。<br />';
                    }
                }
            }
        }
...
__EOF__;
    ...
    /**
     * 求人IDをキーにして, 求人規格の初期値を取得する.
     *
     * 求人IDをキーにし, デフォルトに設定されている求人規格を取得する.
     *
     * @param  integer $product_id 求人ID
     * @return array   求人規格の配列
     */
