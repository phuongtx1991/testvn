 ...
 *
 * FIXME 同一求人IDで求人規格違いを登録できない。(更新は可能)
 */
class LC_Page_Admin_Products_UploadCSV extends LC_Page_Admin_Ex
{
    ...
    public function lfReadCSVFile(&$objFormParam, &$fp, $objQuery = null)
    {
        ...
        while (!feof($fp)) {
            ...
            // 求人IDが設定されており、規格IDが設定されていなければ、既存の規格ID取得を試みる(product_class_idは必須入力項目ではない)
            ...
            if (!$dry_run) {
                ...
                $this->addRowResult($line_count, '求人ID：'.$arrParam['product_id'] . ' / 求人名：' . $arrParam['name']);
            }
     ...
     * 求人登録を行う.
     *
     * FIXME: 求人登録の実処理自体は、LC_Page_Admin_Products_Productと共通化して欲しい。
     *
    ...
    public function lfRegistProduct($objQuery, $line = '', &$objFormParam)
    {
        ...
        // 求人登録情報を生成する。
        // 求人テーブルのカラムに存在しているもののうち、Form投入設定されていないデータは上書きしない。
     ...
     * 求人規格登録を行う.
     *
     * FIXME: 求人規格登録の実処理自体は、LC_Page_Admin_Products_Productと共通化して欲しい。
     *
     * @param  SC_Query $objQuery         SC_Queryインスタンス
     * @param  array    $arrList          求人規格情報配列
     * @param  integer  $product_id       求人ID
     * @param  integer  $product_class_id 求人規格ID
     * @return void
     */
    public function lfRegistProductClass($objQuery, $arrList, $product_id, $product_class_id)
    {
        ...
        // 求人規格登録情報を生成する。
        // 求人規格テーブルのカラムに存在しているもののうち、Form投入設定されていないデータは上書きしない。
    ...
    /**
     * 求人データ登録前に特殊な値の持ち方をする部分のデータ部分の初期値補正を行う
     *
     * @param array $sqlval 求人登録情報配列
    ...
    /**
     * 求人規格データ登録前に特殊な値の持ち方をする部分のデータ部分の初期値補正を行う
     *
     * @param array   $sqlval     求人登録情報配列
    ...
    public function lfCheckErrorDetail($item, $arrErr)
    {
        ...
        if (!$this->lfIsDbRecord('dtb_products_class', 'product_class_id', $item)) {
            $arrErr['product_class_id'] = '※ 指定の求人規格IDは、登録されていません。';
        }
        // 求人ID、規格IDの組合せチェック
        if (array_search('product_class_id', $this->arrFormKeyList) !== FALSE
            ...
            if ($item['product_id'] == '') {
                ...
                if ($product_class_id && $product_class_id != $item['product_class_id']) {
                    $arrErr['product_class_id'] = '※ 指定の求人ID/規格分類と、求人規格IDの組合せは正しくありません。';
                }
            }
        }
        // 表示ステータスの存在チェック
        ...
        if (!$this->lfIsArrayRecord($this->arrProductType, 'product_type_id', $item)) {
            $arrErr['product_type_id'] = '※ 指定の求人種別IDは、登録されていません。';
        }
        // 関連求人IDのチェック
            ...
            if ((array_search($recommend_product_id_key, $this->arrFormKeyList) !== FALSE)
             && ($item[$recommend_product_id_key] != '')) {
                ...
                if (!$this->objDb->sfIsRecord('dtb_products', 'product_id', (array) $item[$recommend_product_id_key])) {
                    $arrErr[$recommend_product_id_key] = "※ 指定の関連求人ID($i)は、登録されていません。";
                    continue;
                }
                // 求人IDの重複チェック
                $recommend_product_id = $item[$recommend_product_id_key];
                if (isset($arrRecommendProductUnique[$recommend_product_id])) {
                    $arrErr[$recommend_product_id_key] = "※ 指定の関連求人ID($i)は、すでに登録されています。";
                } else {
        ...
        if (!$this->lfIsArrayRecordMulti($this->arrSTATUS, 'product_statuses', $item, ',')) {
            $arrErr['product_statuses'] = '※ 指定の求人ステータスIDは、登録されていません。';
        }
        // 削除フラグのチェック
...
*/
        ...
        if (array_search('product_type_id', $this->arrFormKeyList) !== FALSE
            ...
            if ($item['down_filename'] != '') {
                $arrErr['down_filename'] = '※ ダウンロード求人ではない場合、ダウンロードファイル名は入力できません。';
            }
            if ($item['down_realfilename'] != '') {
                $arrErr['down_realfilename'] = '※ ダウンロード求人ではない場合、ダウンロード求人用ファイルアップロードは入力できません。';
            }
            ...
            //ダウンロード求人の場合
            if ($item['down_filename'] == '') {
                $arrErr['down_filename'] = '※ ダウンロード求人の場合はダウンロードファイル名は必須です。';
            }
            if ($item['down_realfilename'] == '') {
                $arrErr['down_realfilename'] = '※ ダウンロード求人の場合はダウンロード求人用ファイルアップロードは必須です。';
            }
        }
