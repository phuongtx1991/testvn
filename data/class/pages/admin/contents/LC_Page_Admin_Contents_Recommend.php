...
class LC_Page_Admin_Contents_Recommend extends LC_Page_Admin_Ex
{
        ...
        switch ($this->getMode()) {
            ...
            case 'regist': // 求人を登録する。
                ...
                if (SC_Utils_Ex::isBlank($this->arrErr)) {
                    ...
                    $arrItems[$rank]['comment_vn'] = $arrPost['comment_vn'];
                    if ($arrPost['best_id']) {
                    } else {
    ...
    {
        $objFormParam->addParam('おすすめ求人ID', 'best_id', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('求人ID', 'product_id', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('コメント_VN', 'comment_vn', LTEXT_LEN, 'KVa', array('EXIST_CHECK', 'MAX_LENGTH_CHECK'));
    }
     ...
     * @return Array  $arrReturnProducts データベースに登録されているおすすめ求人の配列
     */
    ...
    public function insertRecommendProduct($arrPost, $member_id, SC_Helper_BestProducts_Ex &$objRecommend)
    {
        ...
        $sqlval['comment_vn'] = $arrPost['comment_vn'];
     ...
     * @return Array   $return 求人のデータを格納した配列
     */
     ...
     * @param Array $arrItems フロントに表示される求人の情報を格納した配列
     */
