...
class LC_Page_Admin_System_Masterdata extends LC_Page_Admin_Ex {
    ...
    public function action() {
        $this->arrMasterDataName = $this->getMasterDataNames(array('mtb_zip', 'mtb_constants'));
        ...
        if ($this->getMode() != '') {
            ...
            if ($this->masterDataName == 'mtb_city')
                $this->parent = 'region';
            else if ($this->masterDataName == 'mtb_salary_range')
                $this->parent = 'currency';
            else if ($this->masterDataName == 'mtb_pref' || $this->masterDataName == 'mtb_category')
                $this->parent = 'object';
        }
        ...
        switch ($this->getMode()) {
            case 'edit':
                ...
                if (empty($this->errorMessage)) {
                    // 取得したデータからマスターデータを生成
                    $this->registMasterData($_POST, $masterData, $this->masterDataName, $this->parent . '_id');
            ...
            case 'show':
                ...
                if ($this->parent != '') {
                    $this->arrParentMasterData = $masterData->getDbMasterData('mtb_' . $this->parent);
                    $this->arrMasterParentData = $masterData->getDbMasterData($this->masterDataName, array('id', $this->parent . '_id'));
                }
                if (in_array($this->masterDataName, $masterData->arrVnMasterDataName)) {
                    $this->vn = 1;
                    $this->arrMasterVnData = $masterData->getDbMasterData($this->masterDataName, array('id', 'name_vn'));
                } else
                    $this->vn = 0;
                break;
     ...
     * @return void
     */
    public function registMasterData($arrParams, &$masterData, $master_data_name, $parent = '_id') {
        $arrTmp = $this->createKeyVal($arrParams, 'id', 'name');
        ...
        if ($parent != '_id') {
            $arrTmp = $this->createKeyVal($arrParams, 'id', $parent);
            $masterData->updateMasterData($master_data_name, array('id', $parent), $arrTmp, false);
        }

        if (isset($arrParams['name_vn'])) {
            $arrTmp = $this->createKeyVal($arrParams, 'id', 'name_vn');
            $masterData->updateMasterData($master_data_name, array('id', 'name_vn'), $arrTmp, false);
        }

        $masterData->objQuery->commit();
    }

    public function createKeyVal($arrParams, $k, $v) {
        $arrTmp = array();
        foreach ($arrParams[$k] as $key => $val) {
            // ID が空のデータは生成しない
            if ($val != '') {
                $arrTmp[$val] = $arrParams[$v][$key];
            }
        }
        return $arrTmp;
    }
