...
class LC_Page_Entry extends LC_Page_Ex
{
                ...
                if (isset($_POST['submit_address'])) {
                    ...
                    if (count($this->arrErr) == 0) {
                        ...
                        if (!empty($arrAdsList)) {
                            ...
                            $this->arrErr['zip01'] = '※Không tìm thấy địa chỉ tương ứng.<br>';
                        }
                    }
                    break;
                }
    ...
    public function lfSendMail($uniqid, $arrForm)
    {
        ...
        if (CUSTOMER_CONFIRM_MAIL == true) {
            $subject        = $objHelperMail->sfMakeSubject('Xác nhận đăng ký thành viên');
            $toCustomerMail = $objMailText->fetch('mail_templates/customer_mail.tpl');
        } else {
            $subject        = $objHelperMail->sfMakeSubject('Đăng ký thành công');
    ...
    public function lfCheckError($arrRequest)
    {
        ...
        $objFormParam = new SC_FormParam_Ex();
        // パラメーター情報の初期化
        $objFormParam->addParam('ô 1 mã bưu chính', 'zip01', ZIP01_LEN, 'n', array('EXIST_CHECK', 'NUM_COUNT_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('ô 2 mã bưu chính', 'zip02', ZIP02_LEN, 'n', array('EXIST_CHECK', 'NUM_COUNT_CHECK', 'NUM_CHECK'));
        // // リクエスト値をセット
